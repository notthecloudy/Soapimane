...
namespace AILogic
{
    internal class CaptureManager
    {
        ...
        private Vortice.DXGI.IDXGIOutputDuplication? _deskDuplication;

        
        // Performance: Triple-buffering for DirectX
        private ID3D11Texture2D[]? _stagingTextures;
        private int _currentStagingIndex = 0;
        ...
        private Rectangle _lastRoi = Rectangle.Empty;
        private const int ROI_MARGIN = 50;
        ...
        // Performance: Async readback state
        private bool _useAsyncReadback = true;
        ...
        public void HandlePendingDisplayChanges()
        {
            lock (_displayLock)
            {
                ...
                {
                    // Ignore exception - will retry on next frame
                }
            }
        }
        ...
        public void InitializeDxgiDuplication()
        {
            ...
            try
            {
                ...
                Vortice.DXGI.IDXGIOutput1? targetOutput1 = null;
                Vortice.DXGI.IDXGIAdapter1? targetAdapter = null;
                    ...
                    adapterIndex++)
                {
                        ...
                        outputIndex++)
                    {
                        using (output)
                        {
                            var output1 = output.QueryInterface<Vortice.DXGI.IDXGIOutput1>();
                        ...
                        adapterIndex++)
                    {
                            ...
                            outputIndex++)
                        {
                            if (currentIndex == targetIndex)
                            {
                                ...
                                targetOutput1 = output.QueryInterface<Vortice.DXGI.IDXGIOutput1>();
                                targetAdapter = adapter;
        ...
        private Bitmap? DirectX(Rectangle detectionBox)
        {
            ...
            Vortice.DXGI.IDXGIResource? desktopResource = null;
            ...
            // Performance: Initialize triple-buffering if not already done
            InitializeTripleBuffering(w, h);
            
            // Legacy: Check if we need new staging texture - always match requested size
            if (_stagingTex == null ||
                ...
                _stagingTex.Description.Height != h)
            {
                ...
                using (var screenTexture = desktopResource.QueryInterface<ID3D11Texture2D>())
                {
                    ...
                    if (srcRight > srcLeft && srcBottom > srcTop)
                    {
                        ...
                        // Performance: Use triple-buffering if available
                        if (_stagingTextures != null && _useAsyncReadback)
                        {
                            // Rotate through buffers: write to current, read from previous
                            int writeIndex = _currentStagingIndex;
                            int readIndex = (_currentStagingIndex + BUFFER_COUNT - 1) % BUFFER_COUNT;
                            _currentStagingIndex = (_currentStagingIndex + 1) % BUFFER_COUNT;

                            // Copy to write buffer (GPU operation - non-blocking)
                            _dxDevice.ImmediateContext.CopySubresourceRegion(
                                   _stagingTextures[writeIndex], 0,
                                   (uint)(srcLeft - relativeDetectionLeft),
                                   (uint)(srcTop - relativeDetectionTop),
                                   0,
                                   screenTexture, 0, box);

                            // Try async map on read buffer (from previous frame)
                            var mapResult = TryAsyncMap(_stagingTextures[readIndex], out var map);
                            
                            if (mapResult.Success)
                            {
                                // Data ready - process immediately
                                resultBitmap = ProcessMappedData(map, w, h, directXBitmap);
                                _dxDevice.ImmediateContext.Unmap(_stagingTextures[readIndex], 0);
                            }
                            else
                            {
                                // Data not ready - use cached frame or fallback to sync
                                var cached = GetCachedFrame(detectionBox);
                                if (cached != null)
                                {
                                    resultBitmap = cached;
                                }
                                else
                                {
                                    ...
                                    map = _dxDevice.ImmediateContext.Map(_stagingTextures[readIndex], 0, MapMode.Read, MapFlags.None);
                        ...
                        else
                        {
                            ...
                            var map = _dxDevice.ImmediateContext.Map(_stagingTex, 0, MapMode.Read, MapFlags.None);
        ...
        /// <summary>
        /// Initializes triple-buffering staging textures
        /// </summary>
        private void InitializeTripleBuffering(int width, int height)
        {
            if (_stagingTextures != null && 
                _stagingTextures[0]?.Description.Width == (uint)width &&
                _stagingTextures[0]?.Description.Height == (uint)height)
            {
            ...
            _stagingTextures = new ID3D11Texture2D[BUFFER_COUNT];
            
            for (int i = 0; i < BUFFER_COUNT; i++)
            {
                _stagingTextures[i] = _dxDevice!.CreateTexture2D(new Texture2DDescription
                {
                    Width = (uint)width,
                    Height = (uint)height,
                    MipLevels = 1,
            ...
            _currentStagingIndex = 0;
            LogManager.Log(LogLevel.Info, $"Triple-buffering initialized: {width}x{height}");
        }
        
        /// <summary>
        /// Attempts to asynchronously map a staging texture
        /// </summary>
        private SharpGen.Runtime.Result TryAsyncMap(ID3D11Texture2D texture, out MappedSubresource map)
        {
            try
            {
                map = _dxDevice!.ImmediateContext.Map(texture, 0, MapMode.Read, MapFlags.DoNotWait);
            ...
            {
                map = default;
                return Vortice.DXGI.ResultCode.WasStillDrawing;
            }
        }
        ...
        /// <summary>
        /// Processes mapped GPU data into a bitmap
        /// </summary>
        private unsafe Bitmap? ProcessMappedData(MappedSubresource map, int w, int h, Bitmap targetBitmap)
        {
            var boundsRect = new Rectangle(0, 0, w, h);
            BitmapData? mapDest = targetBitmap.LockBits(boundsRect, ImageLockMode.WriteOnly, targetBitmap.PixelFormat);
                ...
                if (Dictionary.toggleState["Third Person Support"])
                {
                    ...
                    for (int y = startY; y < h; y++)
                    {
                        ...
                        for (int x = 0; x < width; x++)
                        {
                            ...
                            rowPtr[pixelOffset + 3] = 255; // Alpha -> 255
                        }
                    }
                }
            }
            ...
            {
                targetBitmap.UnlockBits(mapDest);
            }

            return (Bitmap)targetBitmap.Clone();
        }
        
        /// <summary>
        /// Captures a region of interest around a target
        /// </summary>
        public Bitmap? CaptureROI(Point targetCenter, int targetWidth, int targetHeight)
        {
            // Calculate ROI with margin
            int roiWidth = Math.Min(targetWidth + ROI_MARGIN * 2, DisplayManager.ScreenWidth);
            int roiHeight = Math.Min(targetHeight + ROI_MARGIN * 2, DisplayManager.ScreenHeight);
            int roiX = Math.Max(0, targetCenter.X - roiWidth / 2);
            int roiY = Math.Max(0, targetCenter.Y - roiHeight / 2);
            
            var roi = new Rectangle(roiX, roiY, roiWidth, roiHeight);
            
            // Only update staging texture if ROI size changed significantly
            if (_lastRoi.Width != roiWidth || _lastRoi.Height != roiHeight)
            {
                InitializeTripleBuffering(roiWidth, roiHeight);
                _lastRoi = roi;
            }
        ...
        public void DisposeDxgiResources()
        {
            lock (_displayLock)
            {
                try
                {
                    ...
                    // Dispose triple-buffering textures
                    if (_stagingTextures != null)
                    {
                        foreach (var tex in _stagingTextures)
                        {
