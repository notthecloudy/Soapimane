...
namespace Soapimane.Other
{
    ...
    internal class RequirementsManager
    {
        ...
        {
            if (MessageBox.Show("LG HUB is not running, is it installed?", "Soapimane - LG HUB Mouse Movement", MessageBoxButton.YesNo, MessageBoxImage.Error) == MessageBoxResult.No)
            {
                if (MessageBox.Show("Would you like to install it?", "Soapimane - LG HUB Mouse Movement", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                {
        ...
        {
            if (MessageBox.Show("LG HUB install is improper, would you like to install it?", "Soapimane - LG HUB Mouse Movement", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
                new LGDownloader().Show();
            }
        }
