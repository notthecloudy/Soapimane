...
namespace Soapimane.Other
{
    ...
    internal class UpdateManager
    {
        ...
        public async Task CheckForUpdate(string currentVersion)
        {
            ...
            var releaseInfo = await githubManager.GetLatestReleaseInfo("notthecloudy", "Soapimane");
            string latestVersion = releaseInfo.tagName;
            string latestZipUrl = releaseInfo.downloadUrl;
        ...
        {
            // Download the newest release of Soapimane to %temp%
            string envTempPath = Path.GetTempPath();
            string localZipPath = Path.Combine(envTempPath, "SoapimaneUpdate.zip");
            ...
            string extractPath = Path.Combine(envTempPath, "SoapimaneUpdate");
            ...
            // Create a batch script to move the files and restart Soapimane
